...
const boilerPlateStreamProcessor = async ({ entityName }: CortexStep<any>, stream: AsyncIterable<string>): Promise<AsyncIterable<string>> => {
  ...
  export const outOfCharacter = (extraInstructions?: string, verb = "said") => {
    return () => {
      return {
        command: ({ entityName: name }: CortexStep<any>) => {
          return html`
            You are the omniscient narrator of ${name}'s existence.
    
            ## Instructions
            Do not speak as ${name}, but as a third-person narrator.
            
            ${extraInstructions}
  
            Please reply with your commentary on ${name}'s latest thought, internal or external dialogue.
          `;
        },
